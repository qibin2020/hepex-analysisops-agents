...
from .prompt_builder import build_sam_prompt
from .review import ReviewResult, review_submission_bundle
        ...
        *,
        ...
        last_review = ReviewResult(
            ...
            feedback={
                ...
                "schema_or_type_errors": ["SciFi-OH loop did not run"],
        ...
        for attempt in range(1, self.max_attempts + 1):
            ...
            await self.status(
                f"SciFi-OH loop: Prescan rendered SAM prompt for attempt {attempt}/{self.max_attempts}."
            )
            ...
            if attempt < self.max_attempts:
                await self.status(
                    "SciFi-OH loop: retrying worker with independent review feedback."
                )

        error_payload = {
            "status": "error",
            "error": "SciFi-OH independent review failed before a valid submission_bundle_v1 was produced.",
            "review_feedback": last_review.feedback,
        }
        return SciFiLoopResult(
